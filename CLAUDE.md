...
**Events (`GET /event`).** Subscribing without a directory yields `server.connected` and heartbeats and nothing else — the whole session goes past on a feed nobody is listening to, so `stream` prints its backfill and then sits silent until the session goes idle, and `-H` shows a box that only ever ticks its clock. `subscribeEvents` takes the directory as a required argument; pass `sessionDirectory(meta)`. (`/global/event` carries every project's events wrapped as `{directory, payload}` — a different shape, not a drop-in.)

**Live text arrives as deltas.** The daemon announces a text or reasoning part empty, streams it as `message.part.delta` events (`{messageID, partID, field, delta}`), and only re-sends the finished part as `message.part.updated` at the end of it. A renderer fed on snapshots alone is *technically* live and useless: a whole turn lands in one lump when it is already over. `PartRenderer.delta` accumulates per part; a delta for a part it never saw announced is dropped, since nothing in the event says whether it is prose or a tool's input.

**Events carry both sides of the conversation.** The prompt is published as a part like any other, so the event path filters on message role — without it, `stream` prints the orchestrator's own brief back as though the agent had written it. Backfill filters on role already; events only carry a part and its message id, so the role map is built from `message.updated` (which arrives ahead of its parts) and seeded from the backfill.

**Questions (`GET /question`).** The daemon keeps pending questions per project, resolved from a `directory` query param, and silently falls back to whatever directory it was started in when the param is missing. Every worktree dispatch runs somewhere other than the daemon's cwd, so omitting it makes the question invisible: `/question` returns `[]`, `answer` reports "no pending question", `deriveStatus` never yields `waiting`, and the session sits wedged on an `ask()` that nothing can reach. `listPendingQuestions`/`replyQuestion`/`rejectQuestion` all take a directory — pass `sessionDirectory(meta)`, never a bare daemon URL. `list` covers many sessions at once, so it queries each distinct directory and merges.
